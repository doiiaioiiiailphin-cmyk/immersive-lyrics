...
安装 FFmpeg 并启动网页后端后，打开 `http://127.0.0.1:8765/render/`。渲染器只读取已完整缓存到本机的歌曲，可设置起始和结束时间，支持 1080p、2K、4K 以及 24/30/60 FPS 输出。

渲染任务和历史状态保存在本机 `data/` 目录；刷新页面后仍可查看、取消运行中的任务或下载已完成视频。`data/`、生成视频和所有媒体缓存均被 Git 忽略，不会进入公开源码。
打包 Windows 安装包：
